-- Pre:
-- CREATE UNLOGGED TABLE tmp_snap (LIKE vout INCLUDING ALL);
-- CREATE UNLOGGED TABLE tmp_rid (a_id INTEGER NOT NULL, rid SMALLINT NOT NULL);
-- Timing:
--- snap_fill_daily+daily: 12'20", 80M records
--- snap_fill_month+daily: 14'13", 105M records
...
AS $$
    ...
    ALTER TABLE tmp_snap ADD CONSTRAINT tmp_snap_pkey PRIMARY KEY (t_id,n);
$$;

CREATE OR REPLACE PROCEDURE __snap_unidx()
LANGUAGE sql
AS $$
    ALTER TABLE tmp_snap DROP CONSTRAINT IF EXISTS tmp_snap_pkey;
...
LANGUAGE sql
AS $$
    ALTER TABLE tmp_rid DROP CONSTRAINT IF EXISTS tmp_rid_pkey;
...
AS $$
    ...
    DELETE FROM t_1a_date WHERE t_1a_date.d = d0 AND qid = 1;
...
AS $$
    ...
    DELETE FROM t_1a_date WHERE t_1a_date.d = d0 AND qid = 2;
    INSERT INTO t_1a_date (d, qid, rid, val)
    SELECT
        d0,
        2,
    ...
    INNER JOIN (
        ...
        FROM
            tmp_snap
        WHERE
            (t_id BETWEEN tx0 AND tx1)
            OR (t_id_in BETWEEN tx0 AND tx1)
...
AS $$
    ...
    -- Timing:
    DELETE FROM t_1a_date WHERE t_1a_date.d = d0 AND qid = 3;
    INSERT INTO t_1a_date (d, qid, rid, val)
    SELECT
        d0,
        3,
    ...
    WHERE
        t_id <= tx1 AND
        (t_id_in > tx1 OR t_id_in IS NULL)
    GROUP BY d0, tmp_rid.rid;
$$;
...
AS $$
    ...
    -- Timing: 1'30"
    DELETE FROM t_1a_date WHERE t_1a_date.d = d0 AND qid = 4;
    INSERT INTO t_1a_date (d, qid, rid, val)
    SELECT
        d0,
        4,
...
AS $$
    ...
    DELETE FROM t_1a_date WHERE t_1a_date.d = d0 AND qid = 5;
    INSERT INTO t_1a_date (d, qid, rid, val)
    SELECT
        d0,
        5,
...
AS $$
    ...
    DELETE FROM t_1a_date WHERE t_1a_date.d = d0 AND qid = 6;
    INSERT INTO t_1a_date (d, qid, rid, val)
    SELECT
        d0,
        6,
...
CREATE OR REPLACE PROCEDURE _snap_fill_date(d0 DATE)
LANGUAGE sql
AS $$
    -- Fill tmp_snap with one day vouts
    -- Timing logged:   5'10" (tail)/ ... (vout); 80M records (2022-03)
    -- Timing unlogged: 3'50" (tail)/
    CALL _snap_fill(__get_date_tx_min(d0), __get_date_tx_max(d0));
$$;

CREATE OR REPLACE PROCEDURE _snap_fill_month(d DATE)
LANGUAGE sql
AS $$
    -- Fill tmp_snap with one month vouts
    -- Timing logged:   6'40" (tail)/ ... (vout); 105M records (2022-03)
    -- Timing unlogged: 5'00" (tail)/
    CALL _snap_fill(__get_date_tx_min(__dom_min(d)), __get_date_tx_max(__dom_max(d)));
$$;
...
CREATE OR REPLACE PROCEDURE _monthly(d0 DATE)
...
BEGIN
    ...
    FOR d IN SELECT * FROM generate_series(__dom_min(d0), __dom_max(d0), '1 day')
    LOOP
        CALL _fill_q1a(d, __get_date_tx_min(d), __get_date_tx_max(d));
    END LOOP;
END;
...
CALL _snap_fill_date('2022-03-31');
